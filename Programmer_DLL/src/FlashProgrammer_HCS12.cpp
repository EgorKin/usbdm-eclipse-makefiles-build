...
//!
USBDM_ErrorCode FlashProgrammer_HCS12::getPageAddress(MemoryRegionConstPtr memoryRegionPtr, uint32_t physicalAddress, uint8_t *pageNo) {
   LOGGING_Q;
   *pageNo = 0x00;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS12::setPageRegisters(uint32_t physicalAddress) {
   LOGGING_Q;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS12::resetAndConnectTarget(void) {
   LOGGING;
   USBDM_ErrorCode rc;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS12::readTargetChipId(uint32_t *targetSDID, bool doInit) {
   LOGGING_E;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS12::confirmSDID() {
   LOGGING_E;
   uint32_t targetSDID;
   USBDM_ErrorCode rc;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS12::initialiseTarget() {
   LOGGING;
   USBDM_ErrorCode rc;
   if (initTargetDone) {
...
//! @note Assumes target has been reset & connected
//!
USBDM_ErrorCode FlashProgrammer_HCS12::initialiseTargetFlash() {
   LOGGING;
   USBDM_ErrorCode rc;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS12::loadTargetProgram(FlashOperation flashOperation) {
   LOGGING;
...
//! @note - see loadTargetProgram(MemoryRegionConstPtr, FlashOperation) for details
//!
USBDM_ErrorCode FlashProgrammer_HCS12::loadTargetProgram(MemoryRegionConstPtr memoryRegionPtr, FlashOperation flashOperation) {
   LOGGING_Q;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS12::loadTargetProgram(FlashProgramConstPtr flashProgram, FlashOperation flashOperation) {
   LOGGING;
...
//! +---------------------------------------------------+ -+
//!
USBDM_ErrorCode FlashProgrammer_HCS12::loadLargeTargetProgram(uint8_t    *buffer,
                                                        uint32_t              loadAddress,
                                                        uint32_t              size,
   ...
   headerPtr->copctlAddress = nativeToTarget16(device->getWatchdogAddress());
   ...
   log.print("   flashProgramHeader.copctlAddress   = 0x%08X\n",      targetToNative16(headerPtr->copctlAddress));
...
//! +-----------------------------------------+ -+
//!
USBDM_ErrorCode FlashProgrammer_HCS12::loadSmallTargetProgram(uint8_t    *buffer,
                                                        uint32_t              loadAddress,
                                                        uint32_t              size,
                                                        FlashProgramConstPtr  flashProgram,
                                                        FlashOperation        flashOperation) {
...
//!
const char *FlashProgrammer_HCS12::getProgramActionNames(unsigned int actions) {
unsigned index;
...
//!
const char *FlashProgrammer_HCS12::getProgramCapabilityNames(unsigned int actions) {
   unsigned index;
   static char buff[250] = "";
   static const char *actionTable[] = {
...
USBDM_ErrorCode FlashProgrammer_HCS12::convertTargetErrorCode(FlashDriverError_t rc) {
...
USBDM_ErrorCode FlashProgrammer_HCS12::initLargeTargetBuffer(uint8_t *buffer) {
   LOGGING;
...
USBDM_ErrorCode FlashProgrammer_HCS12::initSmallTargetBuffer(uint8_t *buffer) {
   LOGGING_Q;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS12::executeTargetProgram(uint8_t *pBuffer, uint32_t dataSize) {
   LOGGING;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS12::determineTargetSpeed(void) {
   LOGGING;
...
//! @return error code see \ref USBDM_ErrorCode.
//!
USBDM_ErrorCode FlashProgrammer_HCS12::eraseFlash(void) {
   LOGGING;
   USBDM_ErrorCode rc = BDM_RC_OK;
...
//! @return error code see \ref USBDM_ErrorCode.
//!
USBDM_ErrorCode FlashProgrammer_HCS12::selectiveEraseFlashSecurity(void) {
   LOGGING;
   USBDM_ErrorCode rc = BDM_RC_OK;
...
//! @note - Assumes flash programming code has already been loaded to target.
//!
USBDM_ErrorCode FlashProgrammer_HCS12::partitionFlexNVM() {
   LOGGING;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS12::getTargetBusSpeed(unsigned long *busFrequency) {
   LOGGING;
...
//! @note Assumes the target device has already been opened & USBDM options set.
//!
USBDM_ErrorCode FlashProgrammer_HCS12::checkTargetUnSecured() {
   LOGGING;
   USBDM_ErrorCode rc = initialiseTarget();
   if (rc != PROGRAMMING_RC_OK)
      return rc;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS12::setFlashSecurity(FlashImagePtr flashImage, MemoryRegionConstPtr flashRegion) {
   LOGGING;
...
//! @return error code see \ref USBDM_ErrorCode.
//!
USBDM_ErrorCode FlashProgrammer_HCS12::setFlashSecurity(FlashImagePtr flashImage) {
   LOGGING;
   // Process each flash region
   USBDM_ErrorCode rc = BDM_RC_OK;
...
//! @return error code see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_HCS12::doProgram(FlashImagePtr flashImage) {
   LOGGING;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS12::doSelectiveErase(FlashImagePtr flashImage) {
   LOGGING;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS12::doReadbackVerify(FlashImagePtr flashImage) {
   LOGGING;
...
//! @return error code see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_HCS12::doTargetVerify(FlashImagePtr flashImage) {
   LOGGING;
...
//! @return error code see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_HCS12::doVerify(FlashImagePtr flashImage) {
   LOGGING;
...
//! @return error code see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_HCS12::doBlankCheck(FlashImagePtr flashImage) {
   LOGGING;
...
//! @return error code see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_HCS12::doWriteRam(FlashImagePtr flashImage) {
   LOGGING;
...
//!       locations are ignored.
//!
USBDM_ErrorCode FlashProgrammer_HCS12::verifyFlash(FlashImagePtr flashImage,
...
//!
USBDM_ErrorCode FlashProgrammer_HCS12::programFlash(FlashImagePtr flashImage,
                                              CallBackT    progressCallBack,
                                              bool         doRamWrites) {
   LOGGING;
